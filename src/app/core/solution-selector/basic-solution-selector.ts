...
import {ConcreteSolutionPathStepModel} from '../../data-model/concrete-solution-path-step.model';
import {AbstractAggregatorModel} from '../../data-model/abstract-aggregator.model';

export abstract class BasicSolutionSelector {
  protected abstract getAggregatorRepository(): AbstractAggregatorRepository;
  protected abstract getConcreteSolutionRepository(): AbstractConcreteSolutionRepository;
  ...
  selectConcreteSolutions(patternPath: string[], initialProperties: CapabilityModel[],
    ...
    return this.phase2(phase1CSPaths, initialProperties, userQuery);
  }
  ...
  private getNextPathSteps(currentSolution: ConcreteSolutionModel, nextPatternUri: string): ConcreteSolutionPathStepModel[] {
    const allNextSols: ConcreteSolutionModel[] = this.getConcreteSolutionRepository().getConcreteSolutionsOfPattern(nextPatternUri);
    ...
    if (currentSolution) {// current solution is provided so we are not looking for the starting myNodes
      for (let i = 0; i < allNextSols.length; i++) {
        aggregators = this.getAggregatorRepository().getSuitableAggregators(currentSolution.uri, allNextSols[i].uri);
  ...
  protected phase1(patternPath: string[]): ConcreteSolutionPathModel[] {
    ...
    // console.debug(`phase1 took ${performance.now() - start} ms`);
    return result;
  }
  ...
  protected phase2(solutionPaths: ConcreteSolutionPathModel[], initialProperties: CapabilityModel[],
    ...
    // console.debug(`phase2 took ${performance.now() - start} ms`);
    return result;
  }
