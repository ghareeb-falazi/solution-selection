...
export class SolutionSelectorService {
   ...
   */
  selectConcreteSolutions(patternPath: string[], initialProperties: CapabilityModel[], userQuery: BooleanExpressionModel): ConcreteSolutionPathModel[] {
    let phase1CSPaths: ConcreteSolutionPathModel[] = this.phase1(patternPath);
   ...
   * solution path.
   */
  private depthFirstTraversal(currentStep: ConcreteSolutionPathStepModel, patterPath: string[], currentPosition: number, path: ConcreteSolutionPathModel
    ...
    if (currentPosition >= patterPath.length - 1) {//We have reached the end!
      ...
      const nextSteps: ConcreteSolutionPathStepModel[] = this.getNextPathSteps(currentStep.concreteSolution, patterPath[currentPosition + 1]);
   ...
   * @returns {ConcreteSolutionPathModel[]}
   */
  private static phase2(solutionPaths: ConcreteSolutionPathModel[], initialProperties: CapabilityModel[], userQuery: UserQueryModel): ConcreteSolutionPathModel[] {
